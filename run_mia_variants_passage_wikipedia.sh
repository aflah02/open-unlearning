...
# Usage: ./run_mia_variants_passage_wikipedia.sh <GPU_ID>
GPU_ID=$1
...
for MODEL in "${MODELS[@]}"; do
  ...
  for dup in "${VARIANTS[@]}"; do
    ...
    TASK_NAME="mia_passage_wikipedia_eval_${MODEL_SUFFIX}_dup_${dup}"
    ...
    CUDA_VISIBLE_DEVICES=$GPU_ID python src/eval.py \
        ...
        experiment=eval/mia_passage_wikipedia/default \
        model="$MODEL" \
        task_name="$TASK_NAME" \
        eval.mia_passage_wikipedia.metrics.mia_loss.datasets.forget.args.hf_args.data_files="$DATA_FILE" \
        eval.mia_passage_wikipedia.metrics.mia_min_k.datasets.forget.args.hf_args.data_files="$DATA_FILE" \
        eval.mia_passage_wikipedia.metrics.mia_min_k_plus_plus.datasets.forget.args.hf_args.data_files="$DATA_FILE" \
        eval.mia_passage_wikipedia.metrics.mia_zlib.datasets.forget.args.hf_args.data_files="$DATA_FILE"
