...
for MODEL in "${MODELS[@]}"; do
  ...
  for dup in "${VARIANTS[@]}"; do
    ...
    TASK_NAME="mia_yago_biographies_eval_${MODEL_SUFFIX}_dup_${dup}"
    
    echo ">>> Running on GPU $GPU_ID with model $MODEL and ${dup} duplications"
    echo ">>> Task name: $TASK_NAME"
    
    DATA_FILE="/NS/llm-pretraining/work/afkhan/HubbleSuite/open-unlearning/Hubble_Data/biographies_yago_train_dup_${dup}.jsonl"
    
    CUDA_VISIBLE_DEVICES=$GPU_ID python src/eval.py \
        --config-name=eval.yaml \
        experiment=eval/mia_yago_biographies/default \
        model="$MODEL" \
        task_name="$TASK_NAME" \
        eval.mia_yago_biographies.metrics.mia_loss.datasets.forget.args.hf_args.data_files="$DATA_FILE" \
        eval.mia_yago_biographies.metrics.mia_min_k.datasets.forget.args.hf_args.data_files="$DATA_FILE" \
        eval.mia_yago_biographies.metrics.mia_min_k_plus_plus.datasets.forget.args.hf_args.data_files="$DATA_FILE" \
        eval.mia_yago_biographies.metrics.mia_zlib.datasets.forget.args.hf_args.data_files="$DATA_FILE"
